...
global $conn;
    ...
    //checks whether the member has a delivery address setup
    //get the address_id from the address_details table based on the member_id in auth.php
    $qry_select=mysqli_query($conn,"SELECT * FROM address_details WHERE member_id='$member_id'")
    ...
    if(mysqli_num_rows($qry_select)>0 && isset($_GET['id'])){
           ...
           // date_default_timezone_set($active_reference); //sets the default timezone for use
	        ...
	        //storing the address_id into a variable
	        ...
	        header("location: address-alternative.php"); //redirects to address-alternative.php if not setup
	    }
?>